...
| --- | --- | --- |
| Dispatch entry: load width at +12. | M26 | `metal-microbench/kernels.swift:90` |
| Dispatch entry: load slot stride at +16. | M26 | `metal-microbench/kernels.swift:90` |
| Dispatch entry: load embedding scale at +20. | M26 | `metal-microbench/kernels.swift:91` |
| Dispatch entry: load completed generation at +0; add register stride. | M26 | `metal-microbench/kernels.swift:92` |
| Lane 0 loads lease stop at +8; SIMD broadcast. | M26 | `metal-microbench/kernels.swift:95` |
| Lane 0 loads the directly bound input generation; SIMD broadcast. | M10 | `metal-microbench/kernels.swift:97` |
| Read token from the directly bound canonical input, rank 0 byte offset 12 and other ranks byte offset 0. | M01/M02 | `metal-microbench/kernels.swift:99`, binding at `metal-microbench/mesh_layer.swift:33` |
| Store embedding components into the fixed canonical hidden operand. | M03 | `metal-microbench/kernels.swift:101` |
| After the numerical stores and fence, lane 0 stores completed generation at +0; advance generation in registers. | M26 | `metal-microbench/kernels.swift:104` |
| First remaining numerical consumer loads invocation from its native sequence argument. | M20 | `metal-microbench/kernels.swift:75` |
| First remaining numerical consumer polls completed generation through its native state argument. | M26 | `metal-microbench/kernels.swift:75` |
...
| Sampler loads its tile's fixed 32-byte record before polling. | M12 | `metal-microbench/kernels.swift:3769` |
| Sampler loads sequence through the start-bound argument before polling. | M20 | `metal-microbench/kernels.swift:3773` |
| Sampler polls the prebound availability word. | M10 | `metal-microbench/kernels.swift:3774` |
| Sampler reads its first tile element through the returned coherent data pointer. | M01 or M02 | `metal-microbench/kernels.swift:3759` |
...
| --- | --- |
| Loads listed | 63 sites: 40 emitted AArch64 load instructions and 23 GPU source-level accesses, including 9 resident-prefix sites (4 dispatch-entry, 2 polling, 1 token, 2 consumer handoff). AArch64: TX 6, RX handoff 3, RX continuation 6, link lifecycle 2, numerical notification-to-commit 8, rearm selection 2, rearm descriptor 5, rearm ABI restores 4, numerical lifecycle/backedge 4. GPU source accesses are counted separately from native instructions; no GPU ISA load count is inferred from MSL. Conditional sites count once. |
...
| Construction / compilation | 25/25 listed objects constructed. The resident embedding/scaling prefix and its RMS, embedding and fused QKV consumers compile; it is not the complete resident numerical graph. T3's 26 derivation instructions remain. GPU execution and throughput are unmeasured. |
