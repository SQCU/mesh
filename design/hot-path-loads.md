...
M22 diagnostic samples are native timestamp commands with indices and command-range cuts realized during preparation; `TRACE_COMMANDS` selects individual recorded commands inside caller-selected crossing regions without changing numerical compilation. `TRACE_CROSSINGS` samples one crossing per step and exposes the numerical-region, publication-dispatch and post-publication-to-first-numerical-consumer intervals; a fused publication/consumer is reported as one combined interval, without inferring its internal split; its precise samples perturb the sampled step and do not establish an H7 pass. With the option absent, no cut storage, subrange replay or additional timestamp is submitted; M20 remains the two endpoint samples.
...
Offline placement solving adds no T1/T2 instruction. `tools/mesh/solve_placement.py` reads repeated M22 records after retirement, calls HiGHS on a mixed-integer minimax formulation, and emits caller shares. The existing graph preparer realizes those shares and discards them before execution. M22 repeat counts, replicated cuts and Metal device labels are diagnostic preparation/output only; no profile lookup, solver call or branch selects a shard at runtime.
