...
| Standalone publication dispatch before an eligible native function, whether or not it consumes remote data | M06 removes the recorded publication and binds its M10 records directly to the consumer. Preparation preserves an existing single workgroup unchanged; otherwise it preserves global coordinates while regrouping. It moves the producer barrier with publication and carries resource declarations. One lane publishes; functions with remote operands then use their existing observer. Independent local work retains any original grid, with publication assigned to lane zero of group zero; it adds no observation, cancellation scratch or workgroup barrier. No completion counter or inter-workgroup dependency. |
...
Offline placement solving adds no T1/T2 instruction. `tools/mesh/solve_placement.py` reads repeated M22 records after retirement, calls HiGHS on the cyclic arrival-dependency formulation with caller-supplied handoff measurements, and emits caller shares. Predicted phase offsets remain solver output; they never become runtime delays or a schedule. The existing graph preparer realizes those shares and discards them before execution. M22 repeat counts, replicated cuts and Metal device labels are diagnostic preparation/output only; no profile lookup, solver call or branch selects a shard at runtime.
