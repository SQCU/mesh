...
| --- | --- | --- |
| GPU payload visibility | System-coherent payload reads and writes, system-scope fence, threadgroup barrier | M01, M07 |
...
| Exported full split-half rotary slices, phase temporaries and separate arithmetic dispatches | M06 lowers the expression to upstream `odml.rope` before extraction; prepared sequence/head transposes and a sequence-shaped position operand match the native interface. The original expression remains the composite decomposition, including inverse layout presentation. Geometric frequencies are recognized only at preparation; partial rotations remain their original expressions. |
