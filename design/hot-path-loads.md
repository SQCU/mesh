...
| Packed two- and four-bit weights extracted by float conversion, repeated division, floor and subtraction | M06 compiles equivalent unsigned shifts and masks before the original scale/bias and multiply-accumulate; the packed storage and native output are unchanged |
| Four vector affine weight conversions per quantized-dot iteration with invariant row scale/bias | M06 applies MLX's local dot factorization: accumulate the unsigned-weight dot and the scalar input sum, then apply the existing row scale/bias before the original workgroup reduction. No additional shared-memory load, buffer or dispatch. |
