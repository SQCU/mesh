...
| Standalone publication dispatch before an existing single-workgroup consumer, or a global-index-only consumer whose grid can fit one workgroup | M06 removes the recorded publication and binds its M10 records directly to the consumer. Preparation preserves an existing single workgroup unchanged; otherwise it preserves global coordinates while regrouping. It moves the producer barrier with publication and carries resource declarations. The existing observer publishes before polling; no extra barrier, completion counter or inter-workgroup dependency. |
