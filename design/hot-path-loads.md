...
| Pointwise collective sum dispatch followed by an immediate single-workgroup reader | M06 applies the original pointwise function at the reader’s payload loads, preserving its packed-value rounding and moving the existing publication/completion entry into that native reader. The old sum dispatch, intermediate stores and dependency barrier are removed; the two-layer chain has 75 commands, matching the unpartitioned graph. Preparation requires a private temporary whose next overwrite ends its only reader’s lifetime; no lifetime check executes during replay. |
