...
| Constructor state | `24/24`. M14 is a retired identifier; all listed rows are constructed. |
...
| --- | --- | --- | --- | --- | --- |
| <a id="M01"></a>M01 producer output | `A_rl+s*S(r,l)` | Reserved `S(r,l)`; active `b(r,l)`; scalar width `_Static_assert(sizeof(uint16_t)==2,"M01 M02 M03 scalar storage");` in `rdma/mesh-call.h:35`; extent is the start-bound section byte count. | `metal-microbench/mesh_layer.swift:101`; contiguous slot allocation/binding at `rdma/mesh-call.c:688` and `:695` | value-ready | 0 — native producer binding |
