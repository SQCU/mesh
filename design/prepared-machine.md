...
| `R_r, S_control` | Final canonical control section base and slot stride `Q*ceil(32*B*(35+1)/Q)`; all 36 continuation cells per batch row are allocated before launch. |
...
| Constructor state | `26/26`. M14 is retired. Resident numerical scope: embedding/scaling and all 35 FFNs, including input norm, gate/up, activation, down and publication. Attention, finish/PLE, vocabulary, sampler and T3 remain per-step; the whole-step requirement remains unmet. |
...
| M13 | FFN publication records are consumed only at setup (`metal-microbench/mesh_layer.swift:59`), producing constant offsets and store operands in the resident kernel. Vocabulary/CPU retain one record per native TX cell, local availability word, or CPU notification. TX: destination=M04, value=j+1, scale=0. Local availability: value=1, scale=1. CPU notification: destination=M18+8, value=target+1, scale=2^32. Function constants fix record counts; no header, position, mask or reservation. |
