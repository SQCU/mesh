...
| `R_r, S_control` | Final canonical control section base and slot stride `Q*ceil(C_control/Q)`; `a_finish=32*B*36+sum_l 32*B*(1+K_l+G_l)` and `C_control=a_finish+32*B*35`. `a_l=32*B*36+sum_{i<l}32*B*(1+K_i+G_i)` is the setup attention prefix. `K_l`, `D_l`, `H_l`, `G_l=H_l/QPT_l`, and split count `n_l` are fixed layer dimensions. |
...
| Constructor state | `32/32`. M14 is retired. Resident scope: embedding/scaling and all 35 complete layer bodies, including attention, FFNs, partial summation, norms/residuals, PLE and output scaling. Final model norm, vocabulary, sampler and T3 remain per-step; the whole-step requirement remains unmet. |
...
| --- | --- | --- | --- | --- | --- |
| <a id="M01"></a>M01 producer output | `A_rl+s*S(r,l)` | Reserved `S(r,l)`; active `b(r,l)`; scalar width `_Static_assert(sizeof(uint16_t)==2,"M01 M02 M03 scalar storage");` in `rdma/mesh-call.h:58`; extent is the start-bound section byte count. | `metal-microbench/mesh_layer.swift:386`; contiguous slot allocation/binding at `rdma/mesh-call.c:688` and `:695` | value-ready | 0 — native producer binding |
...
| <a id="M26"></a>M26 resident embedding continuation | `R_r+s*S_control+32*b`, `b<B`; `R_r` is the final canonical storage section allocated for decode | `32` per batch row: completed generation at +0, lease stop +8, width +12, slot stride +16, scale +20. `_Static_assert(sizeof(struct prepared_residency)==32 && offsetof(struct prepared_residency,stop)==8,"M26");` in `rdma/mesh-call.h:35`; matching Metal assertion at `metal-microbench/kernels.swift:72`. | `metal-microbench/mesh_layer.swift:386`, initialized at `:23`; native bindings at `:32`, initial dispatch committed during start-time finalization at `:264` | consumer read | 0 — native buffer binding plus fixed batch-row offset. Width, stride, scale and completed generation load once on dispatch entry, remain in registers across steps. One SIMD group per `(s,b)`; one concurrent command per lease. Lease requests exit after 100 ms, with one 2^22-poll budget per dispatch, never reset per layer; operand polls and numerical tile claims check exit. Claimed tiles and their final normalization/publication complete before exit. Renewal preserves completed generations and does not depend on token arrivals. This bounds the new dispatch's work; it does not establish progress of the remaining per-step graph. |
| <a id="M27"></a>M27 resident FFN continuation | `R_r+s*S_control+32*B*(l+1)+32*b`, `l<35`, `b<B` | `32`: gate next/done at +0/+4, down next/done at +8/+12, input/completed generations at +16/+24. `_Static_assert(4*sizeof(uint32_t)+2*sizeof(uint64_t)==32 && 6*sizeof(uint32_t)+sizeof(uint64_t)==32,"M27 M28 M33");` in `rdma/mesh-call.h:39`; Metal `static_assert(sizeof(mesh_ffn_state)==32,"M27")` at `metal-microbench/kernels.swift:76`. | `metal-microbench/mesh_layer.swift:386`, zeroed at `:55`; resident source and native binding at `:235` and `:257`; attention directly stores its FFN input generation at `metal-microbench/kernels.swift:3170` | consumer read | 0 — native buffer base plus compiled constant offsets. One resident dispatch per slot contains all 35 layers; fixed weight, input, scratch, output and publication offsets are emitted at setup. Existing dot/quantization routines execute 32-column numerical tiles; device atomics claim and complete tiles, with no barrier over dispatched threadgroups. The last down tile directly publishes M04/M10/M18 before its completed generation. M26 stop and the finite poll budget end a lease only between tiles; counters and completed generations survive renewal. |

| <a id="M28"></a>M28 resident attention continuation | `R_r+s*S_control+a_l+32*b`, `b<B` | `32`: project/split/output-next/output-done at +0/+4/+8/+12; input/completed generations +16/+24. `_Static_assert(4*sizeof(uint32_t)+2*sizeof(uint64_t)==32 && 6*sizeof(uint32_t)+sizeof(uint64_t)==32,"M27 M28 M33");` in `rdma/mesh-call.h:39`; Metal assertion at `metal-microbench/kernels.swift:79`. | `metal-microbench/mesh_layer.swift:350`, allocation `:386`, initialization `:98`, bindings `:115`; next-layer input producer `:224` | consumer read | 0 — native base plus compiled layer/row offsets. Attention and FFN stages share the existing resident dispatch, scratch and bounded lease. Projection claims numerical columns; split claims require the corresponding query group and its actual KV producer; output columns consume all reduced heads. Last output tile normalizes, adds the residual and stores M27.input directly. |
| <a id="M29"></a>M29 attention head continuation | `R_r+s*S_control+a_l+32*B+32*(b*(K_l+G_l)+h)`, `h<K_l+G_l`; KV heads first | `32`: column/split completion counts +0/+4, ready/reduced generations +8/+16. `_Static_assert(2*sizeof(uint32_t)+3*sizeof(uint64_t)==32,"M29");` in `rdma/mesh-call.h:41`; Metal assertion at `metal-microbench/kernels.swift:82`. | `metal-microbench/mesh_layer.swift:98`, `:115`; shared-KV producer selected at `:113` | consumer read | 0 — compiled array base, numerical head index. Last projection tile performs that head's norm/RoPE/KV write before ready publication. Last split reduces its query group and publishes reduced; no count of resident threadgroups or host event is used. Shared-KV layers bind the producer layer's head array at setup. |
| <a id="M30"></a>M30 attention split operands | `U_attention_rl+s*Q*ceil(E_l/Q)`; `E_l=4*B*H_l*n_l*(D_l+2)`; maxima +0, denominators `+4*B*H_l*n_l`, weighted values `+8*B*H_l*n_l` | `E_l`; `_Static_assert(sizeof(float)==4,"M30");` in `rdma/mesh-call.h:43`. | Section extent `metal-microbench/mesh_layer.swift:361`, canonical allocation `:386`, native addresses `:115` | consumer read | 0 — compiled buffer base and row offsets; head/split indexing is the existing numerical reduction. Distinct canonical backing per layer and invocation slot. |
| <a id="M31"></a>M31 attention index operands | Each start-bound `Index_rq`, where q names positions, write-skip, lengths, block table or full/partial CSR offsets/indices; row address `Index_rq+b*stride_q` | `4*elements_q` from each existing buffer's setup capacity; `_Static_assert(sizeof(uint32_t)==4 && sizeof(uint64_t)==8 && sizeof(uint16_t)==2,"M31 M32");` in `rdma/mesh-call.h:46`. | Existing operands retained at `metal-microbench/mesh_layer.swift:99`, fixed row bindings at `:115` | consumer read | 0 — compiled operand bases. Position, page and mask values remain numerical inputs to the existing RoPE, KV scatter and attention gather; they do not select collective bindings. |
| <a id="M32"></a>M32 KV chunk directory and backing | Directory `Karg_rl+8*c` / `Varg_rl+8*c`; chunk backing `K_rlc` / `V_rlc`; every c and alias bound at setup, shared-KV l resolves its producer at setup | Directory `8*chunkCount_l`; each chunk `2*chunkPages*16*K_l*D_l` active bytes; scalar/pointer assertion in `rdma/mesh-call.h:46`. | Actual cache resources retained at `metal-microbench/mesh_layer.swift:99`, native directory bindings at `:115` | consumer read | 0 to directory, 1 to chunk backing through the existing numerical KV operand. RoPE, KV scatter, paged attention and split reduction have one shared arithmetic implementation used by native and resident entries. |

| <a id="M33"></a>M33 resident layer-finish continuation | `R_r+s*S_control+a_finish+32*(l*B+b)`, `l<35`, `b<B` | `32`: input next/done +0/+4, gate next/done +8/+12, down next/done +16/+20, completed generation +24. The M27/M28/M33 extent assertion at `rdma/mesh-call.h:39` includes `6*sizeof(uint32_t)+sizeof(uint64_t)==32`; Metal `static_assert(sizeof(mesh_finish_state)==32,"M33")` at `metal-microbench/kernels.swift:85`. | Prefix `metal-microbench/mesh_layer.swift:358`, canonical allocation `:386`, initialization `:159`, compiled operand bindings `:208`; final-norm binding `:435` | consumer read | 0 — native base plus compiled layer/row offsets. Partial stamps and payloads are literal bindings, consumed in arrival order into threadgroup arithmetic. PLE uses the original embedding and canonical token input; existing dot, activation and RMS arithmetic applies the layer projection, token embedding, gate, down projection, norms, residuals and scale. Input-done has one extra final-normalization count. Last down tile publishes the next M28.input and M33.completed; renewal starts from the final M33.completed. One shared poll budget and M26 stop apply between claims and on every peer poll; a claimed tile finishes its stores and publication before exit. |
...
| M12 | Vocabulary payload/availability aliases, element offset and invocation sequence address. Sampler loads its tile record and sequence before polling. Resident FFN consumers use compiled literal bindings and a register generation. |
...
| T1.01b | Vocabulary signal loads its M13 TX record at the encoded address. FFN has no runtime publication-record load. | M13 |
...
| T2.05 | FFN consumer loads generation at each compiled peer stamp address; used-peer flags and selection remain in registers. Vocabulary sampler still uses M12 bindings. | M10 |
