...
| `R_r, S_control` | Final canonical control section base and slot stride `Q*ceil(C_control/Q)`; `C_control=32*B*36+sum_l 32*B*(1+K_l+G_l)`. `a_l=32*B*36+sum_{i<l}32*B*(1+K_i+G_i)` is the setup attention prefix. `K_l`, `D_l`, `H_l`, `G_l=H_l/QPT_l`, and split count `n_l` are fixed layer dimensions. |
...
| Constructor state | `31/31`. M14 is retired. Resident numerical scope: embedding/scaling, all 35 attention blocks (QKV, head norm/RoPE, KV write, split attention/reduction, output projection, post-attention norm/residual), and all 35 FFNs. Finish/PLE, vocabulary, sampler and T3 remain per-step; the whole-step requirement remains unmet. |
...
| --- | --- | --- | --- | --- | --- |
| <a id="M01"></a>M01 producer output | `A_rl+s*S(r,l)` | Reserved `S(r,l)`; active `b(r,l)`; scalar width `_Static_assert(sizeof(uint16_t)==2,"M01 M02 M03 scalar storage");` in `rdma/mesh-call.h:57`; extent is the start-bound section byte count. | `metal-microbench/mesh_layer.swift:349`; contiguous slot allocation/binding at `rdma/mesh-call.c:688` and `:695` | value-ready | 0 — native producer binding |
