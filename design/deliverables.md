...
an unpublished reservation delayed unrelated published entries. ABI 68 removes
that FIFO dependency using independent prepared slots, but its O(E) polling
does not meet H1's complete structure/cost target. Both immediate independent
drainage and the original latency/load budgets remain required. This interim
implementation is not a replacement definition of completion.
...
| W1 | publish path waitless/guardless | ◐ (direct publication; full hot-path budget open) | ABI 68 prepares each row's TX/local destination range and each buffer's return slot. Publication performs one release store per destination without reservation, capacity query or bit reconstruction; TX stores still precede presence and local arrivals. ABI 67's delayed-head dependency is removed. Destination metadata loads and consumer polling still count against H1–H3/H7. [Publication analysis](pages-and-functions.md#publication-notifications). |
...
| W6 | invocation path | ◐ | Consumer records directly name prepared calls; the former join hashes and native-slot rings are deleted. Each aligned 64-byte use record now contains the submit target, argument and operand array/counts directly. Shared-input fan-out and initial/recurring dependency counts are expanded at setup; the arrival classification branch, function/call-array traversal and `mesh_call_ready` lookup are deleted. Mutable dependency state remains a separate access. Setup and final output return prepare ownership and presence; launch only propagates invocation, records an active call and invokes the supplied function after pending reaches zero. Input references end at their own native completion. Metal rearming remains on the return path; Core ML reuses prepared providers/options. Notification and remote-input binding costs, N1 reuse and R2 cancellation remain open. [Local ownership derivation](pages-and-functions.md#output-ownership-is-prepared-before-launch). |
...
| H1 | index rings are the only hot-path interface (three kinds; one store per hand-off) | ✗ (FIFO defect removed; target not met) | ABI 68 removes the FIFO ticket, head dependency and generation check. Setup assigns independent packed 32-bit event slots and contiguous per-row destination ranges; publication is one release store per destination. Link/worker masks and return-queue reconstruction are deleted. Consumers continue past empty slots. **Unfinished:** a pass can probe E subscribed slots; one-line end-to-end latency and complete hot/cold separation are not established. [Handoff analysis](pages-and-functions.md#publication-notifications). |
| H2 | Directly indexed records; native WR/SGE prepared with actual ABI sizes asserted | ◐ | RX WR/SGE prepared per physical block (64 B); SEND WR/SGE prepared per edge (256 B, common prefix and SGE within 128 B). `link_post` deleted; extents fixed at setup. Complete native SEND WR+SGE ≤ 64 B withdrawn because the WR alone is 128 B. Use records pack the submit target, arguments, invocation source and optional remote binding in 64 aligned bytes; shared fan-out is expanded at setup. Call state remains aligned/asserted at 64 B, replacing the 40 B stride; `mesh_call_input`, placement flags and common-path function/frame lookup deleted. Operand row/index/page-list bindings move to setup. ABI 65 prepares completion ranges for SEND, receive publication and buffer return; the buffer record is now 64 aligned bytes with prepared return offset and publication counts, RX remains 32 bytes and SEND remains 256 bytes. TX still binds backing address/key; event polling and one-record event paths remain incomplete. [Transport audit](w-audit-2026-09-15.md#w3), [numerical flow](pages-and-functions.md#prepared-numerical-uses). |
| H3 | the four hot flows exactly as listed (value ready / TX / RX / worker) and the cold pass | ✗ | ABI 63 deletes the per-publication all-queue CQ scan: newly queued sends go directly to their selected queue; capacity refusal returns to CQ progress. It also removes the redundant page-list descriptor load. The earlier 10/21 dependent-load counts are historical; there is no new timing trace. ABI 68 removes hierarchical notices and the FIFO reservation-order dependency, and prepares publication/return destinations. Event polling, tag/address preparation and incomplete cold separation remain. |
| H4 | permission never costs a load (ring empty, verbs return, pending == 0 only) | ◐ | X4 deleted software posting gates; row 19r removed one-post-per-pass. ABI 63's immediate publication post branches only on the native result. ABI 68 removes sequence admission entirely: an empty event slot advances the polling cursor. No producer occupancy check or capacity gate is added. Complete hot-flow work and polling cost remain H1–H3. |
| H5 | dependent loads last in line (hot stores precede them in every hot function) | ✗ | ABI 64 removes the first-chunk flag, conditional, ownership addition and presence clear. ABI 65 removes TX/RX reference-category dispatch and moves frame-identity recording to storage return; numerical/RX return share the buffer-owned reset. Launch no longer loads the function/program or updates output ownership/presence: setup and final output return prepare those counts. Invocation propagation and active-call bookkeeping remain on launch; transport tag/address preparation and other hot-path metadata still prevent completion. |
...
| H8 | the named deletions | ◐ | ABI 63 removes `buffer.mapping` and `mesh_buffer_pages` outright, with no replacement descriptor or wrapper. Earlier hash/definition/slot-stack deletions remain. Notice levels, runtime page resolution, contiguous-input copies and other listed structures still prevent completion. |
