...
| W1 | publish path waitless/guardless | ◐ (publisher does not wait; ABI-67 ring has reservation-order delay) | ABI 67 replaces ABI-48 summary bitmaps with direct index slots; publishers reserve with one fetch-add and release-store one packed event. No CAS retry, linked queue, backing traversal or reader query. The consumer can still be delayed by an unpublished head reservation, so the stronger drainage requirement remains unfinished. ABI 49 final-reference free-pool bits and per-row presence remain. [Publication proof](pages-and-functions.md#publication-notifications), [reclamation proof and limits](pages-and-functions.md#reclamation-events). X5's reusable-instance pool remains open. |
...
| H1 | index rings are the only hot-path interface (three kinds; one store per hand-off) | ◐ | ABI 67 deletes every hierarchical notice level and decoder. TX, numerical/root arrivals and final-reference returns use one index-ring representation: one fetch-add plus one release store per publication; one acquire word plus one head store per consumed event. Index and generation share an 8-byte slot. Head/tail are separately aligned, capacity fixed at attachment, no fullness check. **Unfinished:** an unpublished head reservation delays later entries; three-kind hot/cold separation and immediate independent drainage are not established. [Handoff analysis](pages-and-functions.md#publication-notifications). |
| H2 | Directly indexed records; native WR/SGE prepared with actual ABI sizes asserted | ◐ | RX WR/SGE prepared per physical block (64 B); SEND WR/SGE prepared per edge (256 B, common prefix and SGE within 128 B). `link_post` deleted; extents fixed at setup. Complete native SEND WR+SGE ≤ 64 B withdrawn because the WR alone is 128 B. Use records pack the submit target, arguments, invocation source and optional remote binding in 64 aligned bytes; shared fan-out is expanded at setup. Call state remains aligned/asserted at 64 B, replacing the 40 B stride; `mesh_call_input`, placement flags and common-path function/frame lookup deleted. Operand row/index/page-list bindings move to setup. ABI 65 prepares completion ranges for SEND, receive publication and buffer return; the buffer record is 48 bytes, RX remains 32 bytes and SEND remains 256 bytes. TX still binds backing address/key; H1 independent drainage and one-record event paths remain incomplete. [Transport audit](w-audit-2026-09-15.md#w3), [numerical flow](pages-and-functions.md#prepared-numerical-uses). |
| H3 | the four hot flows exactly as listed (value ready / TX / RX / worker) and the cold pass | ✗ | ABI 63 deletes the per-publication all-queue CQ scan: newly queued sends go directly to their selected queue; capacity refusal returns to CQ progress. It also removes the redundant page-list descriptor load. The earlier 10/21 dependent-load counts are historical; there is no new timing trace. ABI 67 removes hierarchical notices; reservation-order delay, tag/address preparation and incomplete cold separation remain. |
| H4 | permission never costs a load (ring empty, verbs return, pending == 0 only) | ◐ | X4 deleted software posting gates; row 19r removed one-post-per-pass. ABI 63's immediate publication post branches only on the native result. ABI 67 checks the next packed slot generation; an unpublished reservation can delay a later publication. Complete hot-flow record and independent drainage work remains H1–H3. |
