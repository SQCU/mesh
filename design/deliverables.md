...
## Definition — partial tensors use the same supplied functions

The library is an ABI for passing contiguous input/output subviews of large
operands between supplied tensor functions, locally and over RDMA. A partial
changes the operand scope; it does not create a different class of function.
Storage, placement, indexing and communication are realized before execution.

For the user's linear construction, let `sum_i Q_i = I` and `P_i = Q_i X`.
Then `T(X) = sum_i T(P_i)`: every term uses the same T. For invertible T, the
split of an already computed `t = T(X)` is `S(t) = (T Q_i T^-1 t)_i`, with
`R(a_1, ..., a_n) = sum_i a_i` and `R S = I`. Mesh does not implement or infer
T, its inverse, projections or addition; those are supplied tensor operations.
The [derivation](distributed-reduce.md#l5-same-function-different-operand-scopes)
connects that algebra to the source contract.

Domain contributions and disjoint output sections are both ordinary operands.
The caller's functions and indices preserve their coordinate meaning. A partial
may feed another ordinary function before recombination. A nonlinear function
need not distribute over a sum, but that algebraic fact grants Mesh no authority
to reject the call, rewrite its inputs or insert a reduction. The caller declares
the intended dataflow. Numerical association is likewise a caller decision.

L5 and F1 previously prescribed contribution flags, function-call rejection and
layout typing. Those requirements were contrary to this definition and are
superseded below. The ABI has no numerical permission type, privileged combine
path, sum-membership registry or function-category interpreter.
...
| I7 | Mesh contains no numerics or numerical permission system. Matmul, activation, norm, sum, max, attention and sampling are ordinary supplied `TensorFunction`s applied to caller-declared operand scopes. | removing supplied functions leaves only operand representation and communication; no arithmetic fallback, function classifier, contribution flag or privileged combine path |
...
| I23 | Operand scope is preserved through calls and communication. The same supplied function accepts the scopes named by the caller; Mesh does not infer sums, invert functions, re-split values, or add arithmetic. The user's linear decomposition is one exact composition of this ABI. | sends preserve the declared bytes and coordinates; reduction binds its supplied function through ordinary `call`; no numerical classification or sum-membership record exists |
...
Mesh.tensor(on:sections:) -> [TensorPart]                 // contiguous operand scopes with explicit placement
...
**L1. Partial tensor = operand scopes with contiguous storage and placement.** `TensorPart(rank, bytes)`; `Mesh.tensor(on:sections:)`. A large operand is presented as individually consumable contiguous subviews with explicit coordinate/index meaning. These are operands, not numerical permission types. Ref: Pallas BlockSpec. Check: sections name actual registered backing; no hidden dense copy or partial/contribution classification (I4, H6).

**L2. The same supplied function over different operand scopes.** `TensorFunction.cpu/.metal/.prediction`; `Mesh.call/map`. Changing the input/output scope does not create a different class of function, numerical dispatcher or binding privilege. Backend and native-view preparation happen before execution. Ref: Pallas `pallas_call`; StarPU codelets. Check: I7; CPU, Metal and Core ML functions use the same `call` for full operands, partials and reduction contributions; no expression compiler or numerical-function classifier.
...
**L4. Ten explicit collective verbs.** §2 list; movement verbs declare sends; `reduce*` additionally take `using: TensorFunction` and bind it through ordinary `call`. Ref: MPI-4.1 ch. 5; Rabenseifner/Patarasuk–Yuan; Gloo. Check: movement does no arithmetic; reductions use supplied numerics; destinations are explicit at every world size. No implicit gather, reduction, or verb inference from rank count.

**L5. Operand scope never changes call permission.** For `P_i = Q_i X`, `sum_i Q_i = I`, linearity gives `T(X) = sum_i T(P_i)` with the same supplied T in every call. A contribution may feed any further supplied function according to the caller's explicit dataflow. Mesh binds scopes and moves bytes; it neither infers the numerical algebra nor requires recombination before ordinary calls. Check: no contribution flag, contribution registry, call rejection, privileged combine binder or identity replacement to authorize a call. The [algebra and source correction](distributed-reduce.md#l5-same-function-different-operand-scopes) supersede the former L5 restriction.

**L6. Combine is an ordinary supplied function.** `reduce(…, using:)` declares its explicit binary tree through public `call`; the caller supplies an associative operation appropriate to that tree. Ref: `MPI_Op`; Blelloch. Check: no special case for `+`, separate function category, or privileged execution path; a one-input reduction uses the existing value and send.
...
### F — caller-declared scale-out dataflow and performance constraints
...
**F1. Layout and scope are caller-supplied operands.** R/S/P are notation for the caller's algebra, not Mesh types or permissions. The caller declares sections, indices, output scopes and movement/recombination edges. Mesh applies the same supplied functions to those scopes; it does not infer whether a function is pointwise, linear, a norm, or a reduction. Satisfied when: no R/S/P classifier, semantic rejection or alternate function binder exists; realized views preserve the declared coordinates and contiguity. L5 removes the former rejection requirement; H6 still records unfinished contiguous receive placement.

**F2. Communication follows the declared dataflow.** The caller's algebra determines where communication and recombination occur; counts come from those explicit edges. Mesh does not reconstruct a typed numerical chain or infer a collective from an operand category. Satisfied when: the realized sends and calls implement the declared collective verbs with no additional whole-operand gather, reduction or synchronization boundary. Any crossing-count analysis belongs to caller configuration and introduces no runtime interpreter.
...
| 14 | L5 same function over operand scopes | ✓ source correction, 2026-09-16 | Former contribution typing was opposed to the user's algebra. Deleted the flag, registry, rejection closure, private function-binding bypass and reduction-result identity replacement. `map` and reduction's supplied combine both use public `call`. [Derivation](distributed-reduce.md#l5-same-function-different-operand-scopes). Setup deletion only; H runtime defects remain open. |
...
| F1 | caller-supplied operand scopes and layouts | ◐ | R/S/P rejection is removed from the mandate; Mesh has one ordinary function-binding path. The remaining obligation is faithful contiguous operand presentation, tracked by H6, not implementation of a type checker. |
| F2 | communication follows caller-declared dataflow | ◐ | The typed-chain analysis requirement is removed. Collective declarations are explicit; the runtime overhead and implicit-boundary audit remains open under H1–H8 and W1–W8. |
...
| W6 | invocation path | ◐ | ABI 69 events directly index aligned 64-byte use records; row-offset tables become discarded setup scratch. Dispatch no longer binds remote addresses or writes every operand/output sequence. Operands are immutable 48-byte descriptors over canonical page entries and the call sequence; publication writes each output sequence once. Native view objects remain prepared. The actual dependency decrement and active-lifetime accounting precede the direct supplied call. Stream polling, full N1 reuse and R2 cancellation remain open. [Address and invocation analysis](pages-and-functions.md#operand-addresses-and-sequence-values). |
