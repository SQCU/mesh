...
| E1d | the decode layer has one push: attention/norms/residual/PLE duplicated on every rank, FFN columns + vocab tiles split ∝ bandwidth, `down_proj` and logit partials pushed directly to every rank (≤ 120 KiB), summed on arrival; 1 resident command buffer per rank per step | ✗ | engine `00f6e14`: full local attention, rank-local FFN column slices using existing numerical functions, direct `Mesh.send` of each partial to every consumer, local sum before norm/residual/PLE. The sum now binds canonical GPU operands during setup and resolves contiguous received sections inside the existing add kernel. Still two command buffers per layer; the sum is dispatched after all inputs arrive. X9 resident consumption, early GPU publication, vocabulary sharding and complete follower KV initialization remain open. Engine builds and B=1 placement generation pass; no workload or floor-latency measurement ran. The requirement remains unmet. |
...
| 19m | X9 device-readable stamps; resident-kernel consumers (the only way condition 1 of FFN-only TP scale-out holds) | supplanted by H1–H8 (history: ◐ (storage prerequisite only)) | ABI 50 supplies shared 32-bit presence words. `TensorPart.stamp` and canonical CPU/GPU address bindings are now present in ABI 77, and the engine decode sum uses device-side contiguous operand resolution. The separate presence array is removed; the stamp shares a 32-byte canonical address record. The consumer is still dispatched by host readiness, so resident consumption, early GPU publication and measured arrival-to-consumption latency remain unimplemented. [Source account](algorithm-sources.md#device-operands). No GPU visibility or performance claim follows from the CPU word store alone. |
