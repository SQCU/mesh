...
| 19r | post until refused: `link_send_ready`/`link_receive` post ONE work request per queue per progress pass (`if(ready->head!=ready->tail)`, `if(in->next<receive->length)` — X4 replaced `while` by `if`), so the second ready chunk on a queue waits a full spin pass; X4's letter is "post until the verbs call refuses": restore `while` bounded only by the ring/plan and the V return | ✗ | audit 2026-09-16 of `38adbe6`/`d477c6d`: `om:mesh-flow.c:166,:176,:191` |
...
| 19t | receive rows are assigned on arrival by an unguarded per-binding stack pop (`binding->rows[--binding->count]`, `om:mesh-flow.c:214`) driven by an in-band identity tag; silent underflow when a binding's rows are exhausted while the queue's pages remain — the allocation defect the operator named; N1 must plan the row per chunk at `start()` and size rows with pages | ✗ | audit of `d477c6d`; W2 "✓ under corrected I17, ✗ under the planned-row rubric" |
...
| 19u | X10 one-line event records on every runtime path (I22); replaces tag → definition → binding → stack, notice bitmap levels, hash/linear probes, page-table loads at post/receive/invoke | ✗ | ontology audit 2026-09-16 pending; this is the shape N1/19t/19r/19s must take, not a separate mechanism |
