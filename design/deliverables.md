...
| W2 | receive path | ◐ | Dedicated polling, source identity resolution and `mesh_receive_assign` preserve finite out-of-order sections without payload copying. ABI 49/50 repair publication/reclamation. Reusable receives and general interleaved-chunk layout remain open. [Audit](w-audit-2026-09-15.md#w2--receive-path). |
| W3 | send path | ◐ | ABI 48/49 remove publication and reclamation retry stacks. ABI 50 replaces append-only `send_ready` with a bounded circular array and resets send cursors on final completion. Full invocation/RX rearm remains N1/N1t work. Consecutive chunks per send currently support contiguous operands; broader interleaving needs X3 layout support. |
