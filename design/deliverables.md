...
| I21 | A legitimacy measurement is a separable linear-algebra assignment at a shape whose bound is ≥ 1.1× before transport: FFN column split and attention head split at prefill-class row counts (≥ 512 rows: prefill, continued prefill, speculative verification), V ≥ 2 NFEs in flight, placement proportional to measured sustained rates (peer share ≈ r/(1+r)). Small-batch per-layer TP decode on an asymmetric pair and any 50/50 placement on an asymmetric pair are not tests; they are not run at all — not for attribution, not for validation: a measurement of a bad launch pattern (per-layer command buffers at small batch, host round-trips per step) validates nothing and normalizes the pattern. `bounds()`/the Amdahl form with measured r is stated before the run; a shape with bound < 1.1× is refused. | every row-19/E3/E4 evidence line names the shape, the bound and the rates; `report.py` refuses a compare whose declared bound is < 1.1 |
...
| F8 | asymptote T(N) verified at N=2 and N=4 within 15% | ✗ | N=2 inputs now measured: r_ANE = 0.16–0.20 (V ≥ 4), c_tile ≈ 84 µs, tile compute 129–308 µs; predicted bound 1.10–1.13; run pending E5 + E6 binding |
...
| W1 | publish path waitless/guardless | ✓ source | ABI 48 indexed publication notices plus ABI 49 final-reference free-pool bits: no CAS retry, duplicate claim, linked queue, backing traversal or reader query on completion. [Publication proof](pages-and-functions.md#publication-notifications), [reclamation proof and limits](pages-and-functions.md#reclamation-events). X5's reusable-instance pool remains open. |
| W2 | receive path | ✗ | Dedicated polling, source identity resolution and `mesh_receive_assign` are permitted by the operator clarification. Distinct per-queue receive runs and page-table assignment preserve finite out-of-order sections without payload copying. Publication still enters the CAS reclamation path; reusable receives and general interleaved-chunk layout remain open. [Audit](w-audit-2026-09-15.md#w2--receive-path). |
| W3 | send path | ◐ | Page-table indexing is permitted. ABI 48/49 remove publication and reclamation retry stacks. Remaining: finite append-only `send_ready` and missing rearm. Consecutive chunks per send currently support contiguous operands; broader interleaving needs X3 layout support. |
| W4 | submit path | ◐ (notification implemented; N1 open) | Root publication is a fixed worker bitmask walk and a bounded number of atomic ORs into each reader's notification arrays. No allocation, CAS retry, linked entry or reservation. The finite index bound and N1 immediate-busy/reuse surface remain unfinished. |
| W5 | result/collect path | ◐ | `Mesh.result` is one status load. ABI 49 removes the duplicate-enqueue guard, CAS linked stack, deferred retries and collector thread. A final reference publishes one section free bit, consumed by setup allocation. The per-worker instance free-pool pop and arena bound remain X5/N1 work; R2 cancellation remains incomplete. |
| W6 | invocation path | ◐ (notice list removed; lifecycle/layout open) | ABI 48 enumerates pending row bits into the existing consumer ranges and countdown. Received operands still resolve through canonical pages and prepared views. Single-use Metal command buffers, fragmented-to-contiguous layout and native interoperability work remain; no function readiness scan is introduced. |
...
| 19a | X1 dense presence and notifications | ◐ (notification/publication paths implemented; per-section device stamp surface open) | `b850d53` (ABI 48) replaces CAS Treiber notices with per-reader row bits and compact summary arrays. ABI 49 removes W1's remaining reclamation stack. One publication per live row and declared ownership make a pending set sufficient; no per-producer reservation or sequence guard is needed. All numerical/RX/submit/TX paths use it. Presence is still bit-packed; the per-section device-readable word/address surface required by X1/X9 remains open. [Proof](pages-and-functions.md#publication-notifications). |
...
Rows 15 and 16 remain open in the runtime lane. The operator clarified I17/I18: dedicated queue progress and canonical page-table matching are permitted. X3a/X3 require completion of that indexed path and asynchronous layout. X4 admission is implemented at 19d. ABI 48 replaces X1 notifications; ABI 49 removes the reclamation retry stack and repairs W1's source path. X5's per-worker instance rings and arena bound remain at 19e, alongside N1/N1t, R2 and the X1/X9 device-word surface. Other open work includes 19f, 20, 22, 22a, 22b, 25, 27, 28 and 29; the matrix retains all requirements. Lane A's L5 work is integrated; the concurrent N1/N2 attempt is recorded in rows 15/16. P1 records the historical ABI 43 run; row 19h records paired ABI 47 bridges. ABI 49 builds locally with the existing callers and engine Mesh target but has not been deployed or measured. No performance claim is based on ABI 48/49.
