...
| F4 | one hop per layer; direct push below ~120 KiB, RS+AG pipelined above; chosen at start() from (bytes, N) | ✗ (rewritten 2026-09-16 under I25; the former "RS+AG bytes constant in N" row demanded two hops on 3 KiB vectors) | the engine binding today is RS+AG on every partial (4 hops per layer); needs E1d; per-link byte counters absent (T3 branch has them) |
...
| E1d | the decode layer has one push: attention/norms/residual/PLE duplicated on every rank, FFN columns + vocab tiles split ∝ bandwidth, `down_proj` and logit partials pushed directly to every rank (≤ 120 KiB), summed on arrival; 1 resident command buffer per rank per step | ✗ | supplants E1/E1c on the decode path (E1's two-collective layer remains the prefill-class binding only where the byte rule of F4 selects RS+AG); needs X9 (resident consumer) for the 1-command-buffer clause; the push itself is `Mesh.send` to each peer — no new verb |
