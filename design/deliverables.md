...
| E1d | the decode layer has one push: attention/norms/residual/PLE duplicated on every rank, FFN columns + vocab tiles split ∝ bandwidth, `down_proj` and logit partials pushed directly to every rank (≤ 120 KiB), summed on arrival; 1 resident command buffer per rank per step | ✗ | engine `00f6e14`: full local attention, rank-local FFN column slices using existing numerical functions, direct `Mesh.send` of each partial to every consumer, local sum before norm/residual/PLE. Still two command buffers per layer; the sum is dispatched after all inputs arrive. X9 resident consumption, early GPU publication, vocabulary sharding and complete follower KV initialization remain open. Engine builds and B=1 placement generation pass; no workload or floor-latency measurement ran. The requirement remains unmet. |
