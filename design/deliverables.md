...
| E6 | the M4's FFN share on Core ML/ANE (r ≈ 0.25 placement) | ◐ tool half | engine `63477e7` `tools/mesh/compile_ffn_slice.py`: per-layer ML Program of this rank's FFN column slice (three fp16 1×1 convs `(1,K,1,M)`, fp32 GELU·mul asserted on the compiled `model.mil`), output = this rank's down-projection partial (F1 `P`), `manifest.json`; E2B hidden = 1536, rank-1 slice 1228 / 2456 columns; 35 layers 622.6 MB in 3.6 s; `--verify 0` nRMSE 3.46e-4 (ALL) / 9.5e-3 (CPU_ONLY fp16 accumulate). **Open:** the fp32 GELU segment is off-ANE, so the r ≈ 0.25 rate must be re-measured on the M4 with these programs before the placement counts as clearing I21; Swift binding half (`TensorFunction.prediction` for rank 1's segment B) after E5 |
