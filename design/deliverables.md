...
| N1t | the slot-free event re-arms transport: RX re-posts the RECV on each freed receive page; TX send-edge chunk cursor resets per traversal; `send_ready` is a ring — files rdma/mesh-flow.c; depends on rows 19c/19e | ◐ | `bf01584` (ABI 50) implements the TX ring and resets each send cursor at its final native completion before releasing ownership. ABI 52 carries logical invocation IDs independently of source rows and native slots; numerical and transfer retirement use those IDs. RX repost/reuse, matching across reused receive targets and the complete N1 cycle remain open; `submit(inFlight + k)` still cannot complete through the receiver. [Ring proof](pages-and-functions.md#reusable-send-queue), [identity derivation](pages-and-functions.md#invocation-identity-and-storage-reuse). |
...
| 15 | N1 unbounded instances | ◐ **← the single blocker for the row-19 measurement (see 19o)** | `count` remains finite. `ab30da4` (ABI 52) separates logical invocation from each function's prepared native slot, matches published operands by invocation, and releases actual source rows. This replaces consumer-slot-derived remote refresh, consumed-section copies, output-section copies and destructor reconstruction; the library remains 2,272 lines. Existing callers and engine integration build; no runtime claim. The `01f2727` modulo admission attempt remains removed. Per-worker free pools, RX/native rearm, bounded reusable storage and the N1 API remain required. |
