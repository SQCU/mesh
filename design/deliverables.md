...
SEND page/registration resolution, publication streams and complete worker
...
| H8 | the named deletions | ◐ | ABI 63 removes `buffer.mapping` and `mesh_buffer_pages` outright, with no replacement descriptor or wrapper. Earlier hash/definition/slot-stack deletions remain. The notice hierarchy remains deleted. ABI 71 also deletes SEND page/span/offset fields and their readers. Numerical operand page resolution, contiguous-input copies and other listed structures still prevent completion. |
