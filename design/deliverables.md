...
| 16 | N2 Result surface | ◐ | ABI 62 reads one lock-free 16-byte `{value, completed}` snapshot in the existing 32-byte frame. Pending invocations cannot observe a prior success; faults preserve the last completed invocation (F10), correcting ABI 61. RX invocation recording remains after publication. The existing Core ML chain uses the result path on submitting/passive ranks. Control-socket/process events now reach the existing controller; native errors wake it without a transport wait. N1 lifetime/admission, in-flight cancellation and same-program recovery remain unfinished. No killed-peer or performance claim. |
...
| H2 | Directly indexed records; native WR/SGE prepared with actual ABI sizes asserted | ◐ | RX WR/SGE prepared per physical block (64 B); SEND WR/SGE prepared per edge (256 B, common prefix and SGE within 128 B). `link_post` deleted; extents fixed at setup. Complete native SEND WR+SGE ≤ 64 B withdrawn because the WR alone is 128 B. Use records directly name the call, optional remote operand and page entry (32 B); call records are aligned/asserted at 64 B, replacing the 40 B stride; `mesh_call_input`, placement flags and common-path function/frame lookup deleted. Operand row/index/page-list bindings move to setup. TX still binds backing address/key; H1 rings and one-record event paths remain incomplete. [Transport audit](w-audit-2026-09-15.md#w3), [numerical flow](pages-and-functions.md#prepared-numerical-uses). |
| H3 | the four hot flows exactly as listed (value ready / TX / RX / worker) and the cold pass | ✗ | ABI 63 deletes the per-publication all-queue CQ scan: newly queued sends go directly to their selected queue; capacity refusal returns to CQ progress. It also removes the redundant page-list descriptor load. The earlier 10/21 dependent-load counts are historical; there is no new timing trace. Hierarchical notices, tag/address preparation and incomplete cold separation remain. |
| H4 | permission never costs a load (ring empty, verbs return, pending == 0 only) | ◐ | X4 deleted software posting gates; row 19r removed one-post-per-pass. ABI 63's immediate publication post branches only on the native result. Complete hot-flow record and notification work remains H1–H3. |
| H5 | dependent loads last in line (hot stores precede them in every hot function) | ✗ | ABI 64 removes the first-chunk flag, conditional, ownership addition and presence clear; numerical/RX return share the buffer-owned reset. Launch no longer loads the function/program or updates output ownership/presence: setup and final output return prepare those counts. Invocation propagation and active-call bookkeeping remain on launch; transport tag/address preparation and other hot-path metadata still prevent completion. |
...
| 29b | R2 loss observed at CQ/control socket; instances conclude as `Result.link`; slots released | ◐ source observation and retirement | The control socket remains open after pairing. The existing link controller observes EOF/error and client exit through kqueue; native CQ/post errors wake it. It stops progress, joins TX/RX and closes QP/CQ/socket/listener. No healthy-path health query or timer. ABI 62 preserves completed results during faults. Client ownership now covers root/return metadata rows, so dead-client replacement reclaims them through existing retirement. Unissued-use cancellation, same-program slot reuse and the killed-peer run remain open. |
| 29c | R3 bounded re-pair loop while the bridge is up (subsumes 19j) | ◐ listener lifetime repaired | Listener closes on connection teardown and is recreated on the next realization. Still one bounded attempt per client attach; `link_run` returns on zero transfers. Idle pairing, repeated attempts and same-program resumption remain unimplemented. |
