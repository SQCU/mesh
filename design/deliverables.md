...
| 16 | N2 Result surface | ◐ (source in working tree) | one-load `Mesh.result(index)`, per-instance success and native link/function errors implemented at ABI 45; numerical retirement replaces per-call whole-program reference updates; all declared transfers contribute through their final chunk. Missing: remote-caller death detection, driver re-realization, N1 reuse and W1–W6 repairs. No killed-peer or performance claim. |
