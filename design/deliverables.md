...
| 15 | N1 unbounded instances — **instance = an integer frame; every per-instance structure is an array indexed by the frame** (`joins[frame]`, `pending[frame]`, `slot = frame`, `status[frame]`, receive row `= first[t] + (seq mod count)·stride + k·block`); no hash table, no 64-bit invocation key, no per-function slot ring, no stack pop; `submit` returns `.busy` from one load of the frame ring's head/tail; storage for a frame returns to the pool on its final reference (X5) | ◐ **← the single blocker for row 19; the current ABI-5x shape (hash join, definitions/bindings/rows stack) is the regression in 19u and is to be replaced, not extended** | audit 2026-09-16 |
