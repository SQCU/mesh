...
| 29b | R2 loss observed at CQ/control socket; instances conclude as `Result.link`; slots released | ◐ | `link_error` records `port.code`; `Mesh.result` reads it program-wide; per-instance conclusion and slot release on loss unverified (needs the killed-peer run) |
