...
| E1c | segment granularity: a layer's local work between collective points is ONE bound function (three per layer: pre-o_proj [norm→q/k/v→RoPE→attention→o_proj partial], post-attention [norm+residual→gate/up→GELU→down partial], post-FFN [norm+residual]) so a decode step is ≈105 command buffers, not ≈455; kernels unchanged, only the binding boundary moves | ✗ | row 19 arithmetic; lane K/M timelines |
