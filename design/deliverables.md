...
| 16 | N2 Result surface | ◐ | ABI 59 removes the invocation directory and lifecycle queues. ABI 61 embeds the completed invocation in the success word; `result(index)` uses one load and treats another invocation's success as busy. RX records invocation after publication and before producer-reference release; no arrival-time busy store. Existing Core ML chain drives and observes a bounded window on submitting/passive ranks. First errors are preserved; link faults invalidate resident slots. N1 lifetime/admission and failure cancellation remain unfinished; no complete N2 claim. Pending control-event work retains the control socket and observes socket/process events in the link controller; native errors wake it without a transport wait. Cancellation, re-pairing and driver recovery remain unfinished. No killed-peer or performance claim. |
