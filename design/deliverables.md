...
| H1 | index rings are the only hot-path interface (three kinds; one store per hand-off) | ✗ (FIFO defect removed; target not met) | ABI 69 compiles independent single-writer streams and final publication indices. Shared tickets, sequence admission, masks and return-queue reconstruction remain deleted. Grouping/remapping is setup-only. ABI 73 places each stream's cursor/mask and slots in one aligned extent, deleting the stored slot offset and its load. Streams, private reader arrays and worker state occupy separate 128-byte boundaries. **Unfinished:** P streams can still scale with graph size; reader/slot probes, coarse event reservation and complete hot/cold separation remain. The larger reservation is explicitly accounted for. [Handoff analysis](pages-and-functions.md#publication-notifications). |
| H2 | Directly indexed records; native WR/SGE prepared with actual ABI sizes asserted | ◐ | ABI 69 removes the TX and numerical row-to-range lookup: events directly name SEND/use records carrying range ends. SEND remains 256 B, RECV WR/SGE 64 B, receive dispatch 32 B, use/call records 64 B. ABI 75 caches terminal client addresses in canonical entries. ABI 76 operands are 48 B: the 40 B descriptor replaces its copied row with a terminal publication pointer. Publication counts and the first target share an aligned 32 B prefix. Receive records replace row/flag with the same prepared pointer and remain 32 B. Canonical entry and stream reads remain; no dispatch refresh is added. ABI 72 native view selection reads the final view index packed beside the physical page in the canonical 64-bit entry; its Swift indexing closure and page-to-view arithmetic are deleted. ABI 70 events carry the invocation value alongside the prepared index. Uses no longer point to buffer labels. ABI 71 TX retains the supplied invocation in a queued range and posts fully prepared native requests. SEND page/span/offset fields and posting-time address/key resolution are deleted; its native header occupies 88 bytes. All links' record storage is prepared before progress. Received backing is scattered into declared outgoing SGEs before publication. Poll/receive targets share a prepared 64-byte queue record. RECV identifiers hold the registered tag address; completion reads it directly. The queue's placement geometry fits a 40-byte extent aligned to 64; its full state remains 128 bytes. Forwarding descriptors carry each destination and its resolved device key together, specialized for the pool's registration regions; the dependent registration-span lookup is deleted. Completion memoizes its arena/counts in the existing 64-byte call record and publishes before following function ownership. ABI 74 places those calls in one stable array and supplies its final index in native return events; no buffer-binding or call-pointer-table lookup intervenes. Receive binding still reads its prepared scatter. Canonical operand reads and complete one-record event paths remain unfinished. [Address analysis](pages-and-functions.md#operand-addresses-and-sequence-values). |
| H3 | the four hot flows exactly as listed (value ready / TX / RX / worker) and the cold pass | ✗ | ABI 69 emits final SEND/use indices, deletes their offset-table loads and removes remote binding plus both operand/output loops from numerical dispatch. Independent streams replace individual-slot scans. ABI 73 also removes the stream-to-slot address load and separates mutable state belonging to independent owners; the number of streams is unchanged. ABI 70 supplies sequence values in publication events; the persisted buffer write follows the hot event stores. ABI 71 deletes posting-time page/registration resolution and request rewriting. Its forced-inline post consumes a prepared request; one queued range per declared send retains immediate drainage and chunk rotation. Native request storage grows with the number of chunks and is accounted for. Receive completion reads its tag directly from the native identifier. Receive forwarding uses region-specialized destination/key descriptors and arithmetic page placement without a registration lookup or division. ABI 72 stores the prepared view/pool index with its physical page in one canonical word; numerical view selection and receive-pool return consume it directly. ABI 74 return processing addresses its call directly from the event index, removing row-based discovery. ABI 75 prepares terminal client addresses at allocation/receive placement and flat CPU/Core ML/Metal copy records at setup; runtime address/pool/extent reconstruction is deleted from those consumers. The added RX store and storage are accounted for. ABI 76 numerical/RX publication consumes final pointers; counts are inline with targets, removing the lifecycle-buffer read before handoffs. Cleanup is a required tail call after publication, reducing its pre-publication stack frame. Stream polling, canonical/scatter reads and incomplete cold separation remain. No new timing trace is claimed. |
...
| H5 | dependent loads last in line (hot stores precede them in every hot function) | ✗ | ABI 69 deletes both numerical launch operand loops and remote address refresh. The call sequence is shared by its operands. ABI 70 publication sends index/label pairs to TX, writes presence, sends local index/label pairs, then writes the persisted lifecycle label. The label therefore needs no source-buffer read in TX or numerical dispatch. Returned receive pages are posted before logical-row reset and instance-reference cleanup; the shared posting loop is inlined. ABI 71 moves native SEND address binding to configuration/receive placement. ABI 76 publication resolves lifecycle storage only after TX/presence/local stores; cleanup is tail-called afterward. Its pre-publication frame drops from 112 to 64 B, with extra later stack work reported. Active-lifetime accounting, stream metadata, receive-binding scatters and tag stores still precede some hot actions. |
| H6 | contiguity by construction; zero runtime page-table reads | ✗ | ABI 63 deletes `buffer.mapping` and its accessor. Canonical chunk entries are directly indexed by row. ABI 75 stores final client addresses in those entries and prepares copy source/destination/extent records; canonical reads and contiguous-input copies remain. Native view arrays have a fixed contiguous representation. Independent arrival order is preserved. [Current costs](pages-and-functions.md#prepared-operand-addresses). |
...
| H8 | the named deletions | ◐ | ABI 63 removes `buffer.mapping` and `mesh_buffer_pages` outright, with no replacement descriptor or wrapper. Earlier hash/definition/slot-stack deletions remain. The notice hierarchy remains deleted. ABI 71 also deletes SEND page/span/offset fields and their readers. Receive forwarding now deletes its registration-span pointer and dependent lookup; resolved keys reside with the destinations. ABI 72 deletes the stored Swift view-index closure and per-consumer selection arithmetic, plus per-block receive-pool index reconstruction. ABI 73 deletes the stream slot-offset field and its runtime load, packed cross-owner cursor placement and blanket initialization of reserved event slots. ABI 74 deletes the call-pointer table, per-function call allocations, row-to-call return lookup and dummy rows for zero-output completions. ABI 75 also deletes per-reader arena/page address arithmetic, per-copy runtime pool/destination/extent reconstruction, and bridged-array alternatives in native view storage. ABI 76 deletes the lifecycle-buffer publication counts, copied operand row, numerical/RX row-to-publication lookup and separate receive publish flag. Counts prefix the existing target storage. Canonical reads, contiguous-input copies and other listed structures still prevent completion. |
