...
| N1r | slot release on non-submitting ranks: `mesh_calls_start` seeds every instance with a transfer reference that only `mesh_calls_submit` releases, so a rank that never calls `submit` (every rank ≠ 0 under E3) never sees its instances' references reach zero — with N1 slot reuse its slots never free. Fix: the seed reference is released by the root publication's arrival on that rank (the broadcast root section), not by a local `submit` | ✗ | found by lane J, 2026-09-15 16:2x |
| 15 | N1 unbounded instances | ◐ | `01f2727`: `Mesh(…, inFlight:)`, `submit(index) -> Result<Void, MeshError>`, slot = index mod inFlight fixed at start(); slot-free stamp is the refcount→0 event (one acquire load → `.busy`); bounded re-arm from start()-time records; `start()` prints bytes per instance and refuses an undersized arena; no alloc/wait/reader query in submit. Missing: N1t (transport reuse of freed pages — `link_receive` re-post, send cursor reset) which is delivered by rows 19c/19e |
| 16 | N2 Result surface | ◐ | `083ddbb`: `MeshError.link(peer:code:)`, `.function(call:code:)`, `.partialOperand`, `.busy`; `mesh_call_fail` = one release store into `status[slot]`; `Mesh.result(index)` = one acquire load (+ bounded read of the links' `port.code`); never blocks. Missing: the killed-peer run (concludes early, driver re-realizes) |
...
| W2 | receive path | ✗ | X4 removes software admission count/gate; each progress step polls one completion and attempts one post before publication; remaining: sequential receive pages, source-tag read, per-source cursor, `mesh_receive_assign` permutation, sticky `failed` latch; X3a matching is a prerequisite for fixed receives; [audit](w-audit-2026-09-15.md#w2) |
