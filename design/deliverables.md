...
| N1t | the slot-free event re-arms transport: RX re-posts the RECV on each freed receive page; TX send-edge chunk cursor resets per traversal; `send_ready` is a ring — files rdma/mesh-flow.c; depends on rows 19c/19e | ◐ | `bf01584` (ABI 50) implements the TX ring and resets each send cursor at its final native completion before releasing ownership. RX repost/reuse, invocation-ID matching and the complete N1 cycle remain open; `submit(inFlight + k)` still cannot complete through the receiver. [Ring proof](pages-and-functions.md#reusable-send-queue). |
...
| 19o | one-shot prefill instances ≈ 1.5 GB each on rank 0 at 512 rows vs the 2.7 GB `/mesh1` arena → ≤ 1 instance in flight; V ≥ 2 needs N1 reuse or a larger prefill arena (4 GiB bank cap) | ✗ BLOCKING row 19 | lane X 2026-09-17 03:46Z: rank 0 prefill instance = 1,483,204,668 B vs the 2.147 GB `/mesh1` arena → 1 instance per process; the pair completed exactly one 2×256-row tick (1053 ms, first-call inflated: Core ML load + first-instance realization; solo tick ≈ 100 ms) then `mesh prefill instances exhausted` on the public path; 0/8 usable → S undefined; fix = lane Y (scratch shared across layers) and/or N1 reuse; lane W static estimate from `MeshPrefillGraph.plannedBytes` |
| 19p | Mini memory: rank 1 bootstrap RSS 13.5 GB beside 4.7 + 2.7 GB wired arenas on 24 GB → free fell to 0.1 GB; the follower must not realize what it never computes (full-layer attention weights, decode graph sections for rows it does not own) | ✗ (confirmed: Mini min free 0.06 GB, rank-1 RSS 10.3 GB; laptop free 10.5 → 0.1 GB at rank-0 bootstrap) | lane X `prefill2/`; lane W `mini/` memory samples |
| 19q | engine bootstrap prints (`[engine] Mesh … bytes per instance`) go to buffered stdout under `serve.py` and are lost at SIGTERM; they must go to `FileHandle.standardError` (memory `kv_ssd_tier1_works_and_buffered_print_trap`) | ✗ | lane X: per-instance bytes had to be recomputed by hand |
...
| F9 | arena constant in N; printed at start() | ◐ | The finite count reserves operand storage. `Mesh.start()` still lacks N1's bytes-per-instance report and reusable-instance arena bound; N-independence is not demonstrated. |
...
| W2 | receive path | ◐ | Dedicated polling, source identity resolution and `mesh_receive_assign` preserve finite out-of-order sections without payload copying. ABI 49/50 repair publication/reclamation; ABI 51 supplies interleaved-chunk layout. Reusable receives remain open. [Audit](w-audit-2026-09-15.md#w2--receive-path). |
| W3 | send path | ◐ | ABI 48/49 remove publication and reclamation retry stacks. ABI 50 replaces append-only `send_ready` with a bounded circular array and resets send cursors on final completion. Full invocation/RX rearm remains N1/N1t work. ABI 51 rotates unfinished edges after each accepted chunk. X3 supplies canonical contiguous placement within the consuming launch; no whole-section send order remains. |
...
| W6 | invocation path | ◐ (notice list removed; lifecycle open) | ABI 48 enumerates pending row bits into the existing consumer ranges and countdown. Received operands still resolve through canonical pages and prepared views. ABI 51 places fragmented contiguous inputs within the consuming launch. Single-use Metal command buffers and native interoperability/reuse work remain; no function readiness scan is introduced. |
...
| 19c0 | X3a indexed matching for independent producers | ◐ source | `d477c6d` (ABI 51) removes whole-section posting order: per-link/per-queue targets and canonical page assignment handle interleaved chunks; TX rotates after each accepted chunk. The existing Gram chain has a four-rank ring configuration with two peers per node. Source mapping and binding proof is in [block addressing](pages-and-functions.md#block-addressing). The relation still has finite instance identities; N1 reuse and a multi-peer runtime demonstration remain open. |
