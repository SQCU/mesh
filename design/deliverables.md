...
| W1 | publish path waitless/guardless | ✗ (X1 notification fixed; X5 open) | ABI 48 replaces `mesh_notice_push` CAS/list publication with indexed row bits and summary words. Publication still calls `mesh_buffer_release` → `mesh_buffer_enqueue`, whose claim check and CAS reclamation stack require X5. [Proof](pages-and-functions.md#publication-notifications). |
...
| W3 | send path | ✗ | Page-table indexing is permitted. ABI 48 removes the CAS/list publication queue. Remaining: finite append-only `send_ready`, reclamation retry stack and missing rearm. Consecutive chunks per send currently support contiguous operands; broader interleaving needs X3 layout support. |
...
| 19a | X1 dense presence and notifications | ◐ (notification mechanism implemented; W1/X5 open) | `b850d53` (ABI 48) replaces CAS Treiber notices with per-reader row bits and compact summary arrays. One publication per live row and declared ownership make a pending set sufficient; no per-producer reservation or sequence guard is needed. All numerical/RX/submit/TX paths use it. Source publication and reader proof in [pages and functions](pages-and-functions.md#publication-notifications); reclamation still prevents W1 completion. |
...
Rows 15 and 16 remain open in the runtime lane. The operator clarified I17/I18: dedicated queue progress and canonical page-table matching are permitted. X3a/X3 now require completion of that indexed path and asynchronous layout, not immutable native addresses. X4 admission is implemented at 19d; X5 remains at 19e; X1 notification replacement at 19a is implemented in ABI 48, with W1 still dependent on X5. Other open work: 19f (POD descriptor implemented; layout integration and reclamation still need 19c/19e), 20 (caller implemented; W1–W6 open), 22, 22a, 22b, 25, 27, 28, 29. Lane A's L5 work is integrated; the concurrent N1/N2 attempt has been integrated and audited as recorded in rows 15/16. P1 records the historical ABI 43 run; row 19h records the later paired ABI 47 bridges. ABI 48 builds locally with the existing callers and engine Mesh target but has not been deployed or measured by the runtime lane. No current performance claim is based on ABI 48.
