...
| E1d | the decode layer has one push: attention/norms/residual/PLE duplicated on every rank, FFN columns + vocab tiles split ∝ bandwidth, `down_proj` and logit partials pushed directly to every rank (≤ 120 KiB), summed on arrival; 1 resident command buffer per rank per step | ✗ | engine `00f6e14`: full local attention, rank-local FFN column slices using existing numerical functions, direct `Mesh.send` of each partial to every consumer, local sum before norm/residual/PLE. The decode caller now binds one resident Metal step; GPU publication feeds the prepared TX streams after each FFN partial, and the resident sum applies present contributions in any arrival order before nonlinear finishing. The embedding and final hidden placement copies and per-layer host commits are removed. [Source account](algorithm-sources.md#resident-metal). The prefill caller now writes all KV heads on every rank inside its existing stages, with the same KV function as standalone prefill and setup-specialized cache strides. Explicit prefill token-row placement supports B=1; distributed decode always constructs the prefill graph. [Source account](algorithm-sources.md#prefill-kv). Engine `ea45180` deletes the host CSR-offset scan from attention launch sizing: resident recording now uses fixed shape-derived split counts, while the GPU reads the actual page ranges. E2B’s 20 KV-sharing layers no longer compute their unused K/V projections; the same 40-projection deletion applies to solo and Mesh. The attention forwarding wrapper and follower no-op output stage are deleted. [E2B geometry and limits](../../../metal-microbench/docs/async_collectives.md#resident-attention-geometry). Engine `02c223e` vocabulary projection uses explicit caller-owned column ranges and the existing numerical backends on every rank; peers push directly to the existing rank-0 sampler, whose unchanged arithmetic reads canonical tile scopes without a dense gather-copy. [Source and limits](../../../metal-microbench/docs/async_collectives.md#distributed-vocabulary-projection). Replicated sampling and the required per-rank vocabulary pushes, the payload check, imported prefix-cache propagation, numerical KV evidence and measured floor/end-to-end performance remain open. Engine builds and B=1 placement generation pass; no workload or floor-latency measurement ran. ABI 79 deletes duplicate invocation state and repeated publication binding during step recording. The engine also replaces 35 per-layer embedding gathers with one grid dispatch, preserving the layer-major operands for every batch size. [Source and remaining costs](h-audit-2026-09-16.md#publication-without-duplicate-invocation-state). The supplied FFN sum now publishes its local partial on entry using the same Mesh shader publication operand as the standalone signal kernel; its local cooperative barrier replaces the two standalone publication dispatches, deleting 70 dispatches per E2B step without a cross-threadgroup counter. The caller now declares 36 actual outputs instead of 493 intermediates: working storage retains its canonical registered allocation and setup binding, deleting 457 per-step releases, return events and resets ([source account](h-audit-2026-09-16.md#delete-output-lifecycle-work-for-resident-working-storage)). The supplied sum also deletes shared consumed-peer flags and their SIMD barriers, retains peer-table addresses across polls, and specializes shape/peer count at setup, removing 105 encoder calls per step ([source account](../../../metal-microbench/docs/async_collectives.md#resident-sum-state)). The leader now starts directly from the existing metadata publication, deleting its redundant CPU decoder, token copy and intermediate readiness lifecycle; followers retain actual decoding ([source account](../../../metal-microbench/docs/async_collectives.md#direct-leader-step-dependency)). Vocabulary publication and command re-recording remain. No measured latency claim follows. The requirement remains unmet. |
