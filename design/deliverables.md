...
**E1d. The decode layer has one push.** Signature: `bindMeshDecodeLayer(mesh, w, layer:, placement:)` in `metal-microbench`, replacing E1's two-collective layer on the decode path: every rank runs the full attention (all heads, full KV — duplicated), the norms, the residual adds and the PLE path in full; runs gate/up/down on its own FFN column slice (`columns_i ∝ BW_i`, E4); its `down_proj` output is a codomain-shaped partial (Definition) which it `send`s directly to every other rank (`3 KiB × B` per push); each rank sums the arriving partials into its residual with a supplied `add` as they land (any order) and proceeds. The lm_head is split by vocabulary tile the same way, each rank pushing its tile's logits (or its local top-k under the sampler's rule) in the same single hop. No `reduceScatter`, no `allGather`, no second collective on the decode path. All of a rank's step is one resident command buffer (X9/F5) whose consuming kernels spin on the presence stamps of the `N−1` incoming partials. Ref: I25; Megatron FFN column split; MLX `shard_linear` (the local kernel is reused unchanged); NCCL LL (flag co-located with data). Check: the printed plan shows exactly 35 + 1 pushes per rank per step, each ≤ 120 KiB, and 1 command buffer per rank per step; the trace shows `T_mesh − T_w·share_i ≤ 0.5 ms`; attention and norm kernels are the existing ones, run on every rank. Not it: E1's `reduceScatter → norm → allGather` on the decode path; a head split on an MQA model; a hop added to avoid running a norm twice.

**E8. Net improvement on the pair we have, at B = 1 decode.** Signature: `report.py --compare --decode --batch 1 --context 512` on the public endpoint, solo M5 Max vs M5 Max + M4 Pro with E1d and the E4 placement at the *bandwidth* ratio (`r = 273/614 = 0.44`, M4 share 31% of FFN columns and vocabulary tiles; not the compute-rate `r` used for prefill). Zero-overhead bound `1 / (0.31 + 0.69/1.44) = 1.27×`; at the floor (35 pushes × 12 µs) the model gives 1.16×; the objective is `S ≥ 1.10` and the report states `S / bound`. Then the same at B = 8 (bound ≤ 1.42 from the measured B = 8 rates). Ref: I21, I24, I25, [crossover note §4](e2b-crossover-2026-09-16.md#4-how-fast-a-transaction-should-be--the-floor-stage-by-stage). Check: solo at reference class (≥ 160 tok/s) or refused; `S ≥ 1.10` at B = 1; per-forward overhead `≤ 0.5 ms` in the trace; the M4's GPU is never idle > 200 µs between its commands during a step (its 4.4× idle-gap slowdown is a mechanism defect, not a rate). Not it: a 50/50 placement; a run whose solo is one of the 12–15 ms runs; a run explained by "the model is too small".
...
| E7 | the decode crossover measured: `T_solo(B, L)`, `T_mesh(B, L)`, `c`, `B·L×(c)`, `S` at `{4k,16k,64k,128k} × {1, 8}` | ✗ | blocked on F11 and F7; engine `049acba` adds `report.py --decode --context --batch` with actual streamed token IDs and the reference-class refusal (I24); no E7 run is recorded; the crossover note's §5 table is the prediction to be matched within 15% |
...
| E8 | net improvement on M5 Max + M4 Pro at B = 1 decode, E2B: bound 1.27× (zero overhead), 1.16× at the floor, objective `S ≥ 1.10`, overhead ≤ 0.5 ms/forward | ✗ | needs E1d + X9 + H1–H8 at pass values; placement at the bandwidth ratio 0.44 (`placement.py --rate 0.44` for decode; the 0.17 compute-rate placement is prefill's); the historical `solo_report_t4` used four-token responses, including a buffered burst, and does not establish the required steady decode baseline; the new report requires at least 256 timed tokens after the initial emitted batch |
...
| H7 | pass values measured by the trace tool (≤ 200 ns store→doorbell M5, ≤ 100 ns CQ→ring, ≤ 3 lines to launch) | ✗ (no trace tool; static counts stand in) | [audit `6a8d422`](h-audit-2026-09-16.md), baseline depth/lines → target: producer **3/6** → 0–1/1; TX spinner **6–7/8** → 1/1; RX **5/13** → 1/1; worker **6/8–9** → 1/≤3; release **3–4/5–8** → 1/1. Event 1 history 8/8 → 10/12 → 9–10/14: depth flat, lines rising across the ABI 50–75 commits. Each "memoize/bind/prepare" commit added a record on the chain (send_ready range ring, send-binding array, receive geometry record, second edge line for the 128 B WR).  Current scoped update: first-post TX **3–4/6**, producer plus TX **6–7/11**; no latency measurement or complete H pass. [Recount](h-audit-2026-09-16.md#follow-up--direct-first-send-and-deletion-of-contribution-typing). The [current E2B structural analysis](e2b-structural-latency-2026-09-17.md) distinguishes resident GPU consumption from the historical worker path and counts the actual TX/RX/rearm instructions and unresolved dependency stages; no complete H pass. |
