...
metadata as tensor payload. The assembly target generates review evidence, not
an automatic dependency proof. It executes no workload and adds no runtime
...
Publication streams, native operand views and complete worker
paths still have the separately recorded H1–H7 defects. Do not mark an entire
...
| H8 | the named deletions | ◐ | ABI 63 removes `buffer.mapping` and `mesh_buffer_pages` outright, with no replacement descriptor or wrapper. Earlier hash/definition/slot-stack deletions remain. The notice hierarchy remains deleted. ABI 71 also deletes SEND page/span/offset fields and their readers. Receive forwarding now deletes its registration-span pointer and dependent lookup; resolved keys reside with the destinations. Numerical operand page resolution, contiguous-input copies and other listed structures still prevent completion. |
