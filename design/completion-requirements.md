...
| C10 | CoreML storage contract covers the whole operation | Open: investigate same-backing aliases and registration of backend-owned IOSurfaces, including weights and intermediates, as documented in `algorithm-sources.md#ane-backing-identity-and-registration`. Missing allocator injection is not evidence of impossibility. `outputBackings` object identity alone does not establish this contract. |
