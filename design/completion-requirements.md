...
| C10 | CoreML storage contract covers the whole operation | Partial: on both machines `IOSurfaceAddress` wraps an existing 16384-byte `/mesh0` mapping and returns its exact base address; installed Espresso exposes external-surface binding. See `algorithm-sources.md#installed-iosurface-and-espresso-evidence`. Connect actual model surface ownership, weights, intermediates and completion; wrapping alone does not establish ANE execution or coherence. Missing allocator injection is not evidence of impossibility. |
