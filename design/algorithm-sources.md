...
The bridge dedicates one spinning POSIX thread to transmit and one to receive. Each direction owns its posting rings and completion-array segment. The receive thread clears receive ownership before publishing presence, so a forwarding send cannot acquire a page and then have its ownership cleared by receive completion. The control thread configures connections and joins both workers before verbs teardown; numerical dispatch remains in the compute process.

Expression region binding consults the caller’s numerical-function mapping before source generation. This includes internal additions in K-partition accumulation and normalization’s partial-sum tree, which enter through `add.bind` rather than another public grid call. Mesh assigns the resulting Partial marker before binding either implementation.

`gemma_mesh_rmsnorm` binds the engine’s existing RMS normalization (Zhang and Sennrich, *Root Mean Square Layer Normalization*, 2019) to canonical input, FP16 scale and output regions. FP16 uses the existing strided kernel; FP32 uses the existing contiguous kernel. Buffer offsets and pipelines are captured at setup. `streaming-chain.py --normalize --tile-columns 256 --numerics …` uses that engine function after all-gather and before the downstream activation and contraction for the 256-column example input. Each independently ready row region is consumed without waiting for other rows.
