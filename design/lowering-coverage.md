...
| Scatter-add | Base output plus all matching row updates, preserving duplicate contributions and update broadcasting; VJP is identity for base and gather plus sum-to for updates. | Through `a80b48f`, vectors and canonical higher-rank bases use shared indexed_add; multidimensional indices expand routing metadata across trailing row axes while features remain vectors. The incorrect custom flat-address scan is removed. Partition-crossing base reshape aliases use metadata fragments through `a64eb23`; metadata size and storage/launch optimization remain open. |
| Matmul | Broadcast batch dimensions, independent transpose flags; VJP uses two contractions and sum-to for broadcast axes. | Through `103e4d0`, all represented rank>=2 matmuls use shared batched views and `nn.linear`/dot. `a64c782` preserves typed modular integer and boolean truth contractions with K-panel/row progress, replacing the custom FP32 approximation. Floating operations retain native lowering; mixed integral operands are explicitly converted for floating arithmetic. Integer panel vectorization/operand reuse and matched performance remain open. |
...
| Ordering and creation | Arange; argpartition currently emits full argsort with stable index tie-break; Philox normal generation; constants and dimensions. | Through `2e4c7ec`, arange uses shared tiled indexed expressions, including exact signed/unsigned ranges, symbolic range lengths and empty outputs. Dimensions resolve through setup constants; their separate numerical emitter and buffer are removed. Through `ce7a508`, random normal generation also uses shared tiled counter expressions with CPU/Metal emission; the private Philox helper is removed. Ordering retains custom Metal. Preserve sorted-index usefulness and deterministic key/counter mapping. Full-sort quadratic rank counting is a performance gap, not a required algorithm. Arange/random/order are nondifferentiable. |
