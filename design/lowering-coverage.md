...
| Scatter-add | Base output plus all matching updates, preserving duplicate contributions; VJP is identity for base and gather for updates. | Metal scans the complete flattened index vector for each output scalar and accumulates float. This implements flat indices, not arbitrary tuple `.at` indexing. Replace with explicit contribution domains and FP32 partial sums, independently complete by destination region. |
