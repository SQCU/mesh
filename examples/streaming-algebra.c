...
int main(int argc,char **argv) {
  ...
  struct mesh_tensor *sum=tensor(a,4,rows,k,0),*activated=tensor(a,4,rows,k,0),*weights=tensor(a,2,k,n,0);
  ...
  for(size_t i=0;i<4;i++) {
    ...
    check(mesh_algebra_bind(a,MESH_CONTRACT,av,mesh_tensor_view(weights,(uint32_t)(i/2)),cv,1,0));
  }
  for(size_t part=0;part<2;part++) {
    float *data=mesh_tensor_data(weights,(uint32_t)part);
    for(size_t r=0;r<k;r++)for(size_t c=0;c<n;c++)data[r*n+c]=weight(part,r,c);
