...
firewall %s
' \
  "$SLEEP" "$DISPLAYSLEEP" "$FIREWALL" > "$POLICY"
ok "policy $POLICY written (the keeper asserts this from now on)"
...
printf '  Undo by returning to fleet policy: sudo ./install.sh\n\n'
