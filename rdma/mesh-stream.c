...
int main(int argc,char**argv){
  ...
  if(!strcmp(mode,"yell")){
    ...
    struct mesh_ctx cx={0};
    struct mstream in, out, *v[2]={&in,&out};
    mesh_lissen_start(&cx,&in,a,N,1);
    while(in.st==MS_RUN) mesh_turn(&cx,v,1);
    for(size_t i=0;i<N/8;i++) a[i]*=2;
    mesh_yell_start(&cx,&out,a,N,in.node,101);
    while(out.st==MS_RUN) mesh_turn(&cx,&v[1],1);
    ...
    double t0=now();
    mesh_gather(&cx,ga,b,N,1,101);
    mesh_scatter(&cx,sc,a,N,nodes,1,1);
    v[0]=&sc[0]; v[1]=&ga[0];
    while(mesh_turn(&cx,v,2)<2){}
    double dt=now()-t0;
    size_t bad=0;
