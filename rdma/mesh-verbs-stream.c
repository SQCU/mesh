...
int main(int argc,char **argv){
  if(argc<3){ fprintf(stderr,"usage: %s send|receive peer [device] [timeout_seconds]\n",argv[0]); return 2; }
  ...
  if(!sges || !sends || !receives || !completions){ status=1; goto storage; }
  ...
  if(!sending && !status) for(uint32_t page=0;page<STREAM_PAGES;page++){
    const uint64_t *words=(void*)(memory+(size_t)page*STREAM_BYTES);
    for(uint32_t word=0;word<STREAM_BYTES/sizeof *words;word++)
      mismatches+=words[word]!=(((uint64_t)page<<32)^word^UINT64_C(0x6d65736870616765));
  }
  if(mismatches) status=1;
  printf("{\"mode\":\"provider_stream\",\"role\":\"%s\",\"bytes\":%zu,\"page_bytes\":%u,\"posted\":%u,\"completed\":%u,\"status\":%d,\"mismatches\":%llu,\"submit_to_last_seconds\":%.9f,\"first_to_last_seconds\":%.9f,\"first_count\":%u,\"low_count\":%u,\"high_count\":%u,\"first_to_last_bytes\":%llu,\"interior_bytes\":%llu,\"interior_seconds\":%.9f,\"polls\":%llu,\"empty_polls\":%llu,\"posts\":%llu,\"poll_seconds\":%.9f,\"post_seconds\":%.9f}\n",
    argv[1],bytes,STREAM_BYTES,posted,completed,status,(unsigned long long)mismatches,
 ...
 storage:
  free(sges); free(sends); free(receives); free(completions);
 finish:
  while(!down_verbs()){}
