...
int main(int argc,char **argv){
  ...
  struct stat info;
  size_t bytes=RINGS+NRING*MESH_RING*sizeof(struct desc)+sizeof(struct mesh_port_info);
  ...
  struct mesh_port_info *port=mesh_ports(h);
  int paired=alive && atomic_load(&port->phase)==MESH_PAIRED;
  printf("{\"up\":%s,\"ready\":%s,\"paired\":%s,\"bridge_pid\":%llu,\"version\":%u,\"node\":%u,\"pgsz\":%u,\"pool\":%u,\"arena\":%u,\"sent\":%llu,\"recvd\":%llu,\"bad\":%llu,\"code\":%lld,\"domain\":%u}\n",
    alive?"true":"false",paired?"true":"false",paired?"true":"false",(unsigned long long)pid,h->version,h->node,h->pgsz,h->pool,h->arena,
    (unsigned long long)atomic_load(&h->sent),(unsigned long long)atomic_load(&h->recvd),(unsigned long long)atomic_load(&h->bad),
    (long long)port->code,port->domain);
