...
LINK=${MESH_LOOPBACK_LINK:-tcp}
A=${MESH_LOOPBACK_A:-127.0.0.1:18601}; B=${MESH_LOOPBACK_B:-127.0.0.1:18602}
LOG=${MESH_LOG_DIR:-$HOME/.mesh-logs}
mkdir -p "$LOG"
case ${1:-start} in
start)
  "$ROOT/rdma/mesh-flow" -I 0 -M "$PCT" -s /loop0 --link "$LINK,1,$A,$B" > "$LOG/loop0.log" 2>&1 &
  echo $! > "$LOG/loop0.pid"
  "$ROOT/rdma/mesh-flow" -I 1 -M "$PCT" -s /loop1 --link "$LINK,0,$B,$A" > "$LOG/loop1.log" 2>&1 &
  echo $! > "$LOG/loop1.pid"
