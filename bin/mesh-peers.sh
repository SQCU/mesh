...
printf '\n  %-18s %-12s %-5s %-6s %-9s %-8s %-21s %-38s %s\n' NODE MODEL CORES MEM RDMA BRANCH CONVERGED ADDRESS VIA
n=0
for e in $(netstat -rn -f inet6 2>/dev/null | awk -v p="$PREFIX" '$1 ~ "^"p":" {split($1,f,"%"); print f[1]"|"$NF}' | sort -u); do
  a=${e%%|*}; via=${e##*|}; n=$((n+1))
  info=$(printf 'x' | nc -G 2 -w 2 "$a" 8099 2>/dev/null)
  g(){ printf '%s' "$info" | awk -F= -v k="$1" '$1==k{print $2;exit}'; }
  name=$(g name); model=$(g model); cores=$(g cores); mem=$(g memgb); rd=$(g rdma); br=$(g branch); cv=$(g converged)
  [ -n "$info" ] || { name="?"; rd="unreachable"; }
  [ "$a" = "$self" ] && via=self
  printf '  %-18s %-12s %-5s %-6s %-9s %-8s %-21s %-38s %s\n' \
    "${name:-?}" "${model:-?}" "${cores:-?}" "${mem:+${mem}G}" "${rd:-?}" "${br:-?}" "${cv:-?}" "$a" "$via"
done
